...
class WorkSession:
    ...
    def __init__(self):
        ...
        self._heartbeat_cb = None  # async callback(msg, elapsed_seconds)

    def set_heartbeat(self, cb):
        """Set async callback for progress updates: cb(msg: str, elapsed: int)"""
    ...
    async def send(self, user_text: str) -> str:
        ...
        TIMEOUT = 900  # 15 minutes for big tasks
        HEARTBEAT = 60  # voice update every 60 seconds
        ...
        try:
            ...
            # Wait with heartbeat updates
            elapsed = 0
            heartbeat_messages = [
                "Still on it, sir. This one requires some careful work.",
                "Progress continues, sir. Approximately halfway through.",
                "Nearly there, sir. Wrapping up the final steps.",
                "Still working, sir. Complex task — almost done.",
                "Continuing the work, sir. Thank you for your patience.",
                "Still at it, sir. This is a thorough task.",
                "Making good progress, sir. Should not be too much longer.",
                "Still processing, sir. The work is coming along nicely.",
                "Continuing, sir. This task is more involved than expected.",
                "Still here, sir. Finishing up the remaining steps.",
                "Nearly complete, sir. Just a few more steps.",
                "Almost done, sir. Wrapping up now.",
                "Still at it, sir. Thank you for waiting.",
                "Continuing the task, sir. Will report shortly.",
                "Still working, sir. Progress is steady.",
            ]
            heartbeat_index = 0
            ...
            while True:
                try:
                    ...
                    elapsed += HEARTBEAT
                    if elapsed >= TIMEOUT:
                        log.error(f"claude -p timed out after {TIMEOUT}s")
                        self._status = "timeout"
                        try:
                            process.kill()
                        except Exception:
                            pass
                        return f"That task exceeded the time limit, sir. It ran for {elapsed // 60} minutes. The work may be partially complete in the project folder."

                    # Send heartbeat via callback if available
                    msg = heartbeat_messages[heartbeat_index % len(heartbeat_messages)]
                    heartbeat_index += 1
                    log.info(f"Work heartbeat ({elapsed}s): {msg}")
                    if self._heartbeat_cb:
                        try:
                            await self._heartbeat_cb(msg, elapsed)
                        except Exception:
                            pass
